...
const AddMeal = () => {
  ...
  useEffect(() => {
    const data =  fetchUserData(uid).then((data) => {
      setUserData(data)
      setWaterGoal(data.mealGoal)
      let waterConsumedData = data.mealConsumed
      ...
      percent = Math.floor(waterConsumedToday*100/data.mealGoal)
    ...
    const AddMealFunc = (amount) => {
        const data = userData
        NewData = {
          mealConsumed: data.mealConsumed
        }
        NewData.mealConsumed.push({
          date: new Date,
          amount: amount
        })
        ...
        if (Math.floor(NewWaterConsumed*100/data.mealGoal) >= 100){
           NewWaterPercent = 1
        } else {
           NewWaterPercent = Math.floor(NewWaterConsumed*100/data.mealGoal)/100
        }
        console.log(NewWaterConsumed)
        setWaterPercent(Math.floor(NewWaterConsumed*100/data.mealGoal))
    ...
    <ScrollView style={styles.container} contentContainerStyle={{paddingBottom: 60}} >
      ...
      <View style={styles.graphWrap} > 
        <ProgressChart
              ...
              //color: (opacity = 1, dataIndex) => {colorsArray[dataIndex] + (Math.round(opacity*100)).toString() || `rgba(26, 255, 146, ${opacity})`; console.log(colorsArray[dataIndex]+(Math.round(opacity*100)).toString())},
                color: (opacity = 1, dataIndex) => {
                    if (opacity == 0.2){
                        ...
                        opacity = 1;
                        return  `rgba(252,84,47, ${opacity})`;
                    }
                    return '#f7542f' + (Math.round(opacity*100)).toString() || `rgba(26, 255, 146, ${opacity})`;
                },
          ...
          <View style={styles.graphTextWrap} >
              ...
              <Text style={[styles.graphText, {color: '#808080', fontSize: 30, marginTop: -30}]} >{waterConsumed} kcals</Text>
          </View>
      </View>
      <View style={styles.section} >
            ...
            <View style={{width: '100%'}}>
              ...
              <TouchableOpacity style={styles.WaterCardWrap}  onPress={() => AddMealFunc(150)} ><MealCard amount={150} /></TouchableOpacity>
              <TouchableOpacity style={styles.WaterCardWrap} onPress={() => AddMealFunc(250)} ><MealCard amount={250} /></TouchableOpacity>
              <TouchableOpacity style={styles.WaterCardWrap} onPress={() => AddMealFunc(400)} ><MealCard amount={400} /></TouchableOpacity>
              </View>
              <View style={{flexDirection: 'row', height: 100, marginTop: 10 }}>
              <TouchableOpacity style={styles.WaterCardWrap} onPress={() => AddMealFunc(600)} ><MealCard amount={600} /></TouchableOpacity>
              <TouchableOpacity style={styles.WaterCardWrap} onPress={() => AddMealFunc(800)} ><MealCard amount={800} /></TouchableOpacity>
              <TouchableOpacity style={styles.WaterCardWrap} onPress={() => nav.navigate('AddMealCustom', {AddMeal: AddMealFunc})} ><MealCard amount={'Other'} /></TouchableOpacity>
              </View>
            </View>
      </View>
      <View style={styles.section} >
          {/* <Text style={[styles.Header, {marginTop: 20}]} >Goal</Text> */}
          <TouchableOpacity style={styles.endBtn} onPress={() => nav.navigate('MealGoal')}>
          <Text style={styles.endBtnText} >Current Goal: {waterGoal} kcals/day</Text>
